...
$pics = CFile::ResizeImageGet($arResult['DETAIL_PICTURE']['ID'], array('width' => 186, 'height' => 186));
    ...
    <?if (count($arResult['PROPERTIES']['FILES']['VALUE']) > 0): ?>
        <? foreach ($arResult['PROPERTIES']['FILES']['VALUE'] as $photo): ?>
            <div class="small_picture">
                <img src="<?=CFile::ResizeImageGet($photo,array('width' => 49, 'height' => 49))?>" alt=""/>
            </div>
        <? endforeach ?>
    <? endif?>
</div>
...
<table class="option" cellpadding="0" cellspacing="0">
    <?foreach ($arResult['PROPERTIES'] as $prop): ?>
        <? if ($prop['VALUE'] != '' && !is_array($prop['VALUE']) && $prop['NAME'] != 'Базовая единица'): ?>
            <tr>
